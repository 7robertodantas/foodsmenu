...
public class MenuDescriptionHandlerTestIT extends IntegrationTestSupport {
    ...
    public void shouldCreateMenuDescription() {
        List<ItemDto> itemDtos = singletonList(new ItemDto("X-Bacon", asList("Bacon", "Hamburguer", "Cheese"), asSet("10off", "3pay2hamburguer", "3pay2cheese")));
        List<QuantitySaleDto> quantitySaleDtos = asList(new QuantitySaleDto("3pay2hamburguer", "For each 3 hamburguers pay only 2.", "Hamburguer", 3, 1),
                new QuantitySaleDto("3pay2cheese", "For each 3 cheese pay only 2.", "Cheese", 3, 1));
        List<CompositionSaleDto> compositionSaleDtos = singletonList(new CompositionSaleDto("10off", "10% OFF - Must have lettuce and not have bacon.", 0.1, asSet("Lettuce"), asSet("Bacon")));
        ...
        MenuDescriptionDto menuDescription = new MenuDescriptionDto("menu", itemDtos, salesDto, ingredientDtos);
